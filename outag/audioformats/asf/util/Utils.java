package outag.audioformats.asf.util;
...
public class Utils {
    ...
    /** Reads chars out of <code>raf</code> until <code>chars</code> is filled.
     * @param chars - to be filled
     * @param raf - to be read
     * @throws IOException - read error, or file at end before <code>chars</code> is filled. */
    ...
    /** Create a byte[] at the size of <code>byteCount</code> and insert
     * the bytes of <code>value</code> (starting from lowset byte) into it. <br>
    ...
    /** Reads one byte from <code>raf</code> and creates an unsigned value of it.
     * @param raf - The file to read from.
     * @return next 7 bits as number.
     * @throws IOException - read errors. */
    public static int read7Bit(RandomAccessFile raf) throws IOException { return raf.read() & 127; }

    /** Reads 8 bytes, interprets them as an unsigned number and creates a {@link BigInteger}
    ...
    /** Reads a UTF-16 String, which legth is given on the number of characters it consits of. <br>
    ...
    /** Reads a GUID (which is a 16 byte long sequence) from the
    ...
    public static GUID readGUID(RandomAccessFile raf) throws IOException {
        if (raf == null) throw new IllegalArgumentException("Argument must not be null");
    ...
    /** Reads a UTF-16 encoded String, beginning with a 16-bit value representing
     *  the number of bytes needed. The String is terminated with as 16-bit ZERO.
     * @param raf - Input source
     * @return read String.
     * @throws IOException - read errors. */
    ...
    /** Converts the given string into a byte[] in UTF-16LE encoding
     * and checks whether the length doesn't exceed 65535 bytes.
