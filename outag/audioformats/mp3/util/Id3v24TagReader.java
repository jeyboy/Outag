...
import outag.audioformats.mp3.util.id3frames.ApicId3Frame;
import outag.audioformats.mp3.util.id3frames.CommId3Frame;
import outag.audioformats.mp3.util.id3frames.GenericId3Frame;
import outag.audioformats.mp3.util.id3frames.Id3Frame;
import outag.audioformats.mp3.util.id3frames.TextId3Frame;
import outag.audioformats.mp3.util.id3frames.TimeId3Frame;
import outag.audioformats.mp3.util.id3frames.UfidId3Frame;

/** Parses an ID3V2 tag from a given {@link java.nio.ByteBuffer}.<br>
 * It handles the versions 2,3 and 4. */
public class Id3v24TagReader {
	/** Maps the field names of the version 2 frames to the one of version 3. */
	private Hashtable<String, String> conversion22to23;

	/** Creates an instance. */
	public Id3v24TagReader() { initConversionTable(); }
	...
	/** Conversion from id3v22 to id3v23 */
	private void initConversionTable() {
		...
		this.conversion22to23 = new Hashtable<String, String>(65);
	...
	/** Used to skip the extended header in the reading process.
	 * @param data - the buffer containing the extended header. (at current location)
	 * @param version - the ID3V2 version {@link Id3v2Tag#ID3V22}.
	...
	/** Reads an ID3V2 tag from the given {@link ByteBuffer} at its current pointer location.
	 * @param data - ID3V2 tag.
	 * @param ID3Flags - The flags of the tag header.
	 * @param version - Version Flag. (used to handle some version specific implementations).
	 * @return An ID3V2 tag representation.
	 * @throws UnsupportedEncodingException - Thrown on charset conversions, if system does not support them. */
	...
	/** Reads the data of an integer out of the given buffer.<br> Since different
	 * version of ID3V2 tags have a different size definition, the version is needed.
	 * 
	 * @param bb - the buffer containing the integer.
	 * @param version - the ID3V2 version. {@link Id3v2Tag#ID3V22}.
	...
	/** Reads the next 4 byte of the buffer and interprets them as a sync safe integer.
