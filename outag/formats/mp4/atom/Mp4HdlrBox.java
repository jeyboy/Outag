...
import outag.formats.mp4.atom.AbstractMp4Box;
import outag.formats.mp4.atom.Mp4BoxHeader;
import outag.formats.mp4.atom.Mp4HdlrBox;
...
public class Mp4HdlrBox extends AbstractMp4Box {
    ...
    public void processData() throws CannotReadException {
        ...
        try { handlerType = decoder.decode((ByteBuffer) dataBuffer.slice().limit(HANDLER_LENGTH)).toString(); }
        catch (CharacterCodingException cee) { /*Ignore */ }
     ...
     * <p>Doesn't write the child data but uses it to set the header length, only sets the atoms immediate data</p */
