...
public class Mp4EsdsBox {
     ...
     * @throws IOException */
    public Mp4EsdsBox(JBBuffer data) throws IOException {    	
        ...
        data.skip(4);  //Skip Flags

        //Process Section 3 if exists
        if (data.read() == SECTION_THREE) {
            sectionThreeLength = processSectionHeader(data);
            //Skip Other Section 3 data
            data.skip(ES_ID_LENGTH + STREAM_PRIORITY_LENGTH);
        }

        //Process Section 4 (to getFields type and bitrate)
        if (data.read() == SECTION_FOUR) {
            sectionFourLength = processSectionHeader(data);

            kind = kindMap.get(data.read()); //kind (in iTunes)
    ...
    public int getAvgBitrate() { return avgBitrate; }    

    /** Only expect MPG_Audio,
     * TODO shouldn't matter if another type of audio, but something gone wrong if type of video
     * @return the file type for the track */
    public Kind getKind() { return kind; }

    /** Get audio profile, usually AAC Low Complexity
     * @return the audio profile */
    public AudioProfile getAudioProfile() { return audioProfile; }
